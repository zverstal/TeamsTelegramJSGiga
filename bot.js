...
async function summarizeMessages(messages, lastMsgId) {
    try {
        ...
        3. Тема: если тема явно указана или может быть определена из контекста, укажи её. Для RE-сообщений отметь, что тема берётся из контекста предыдущего сообщения.
        4. Содержание: составь одно-два предложения, точно описывающих содержание сообщения. Обязательно включи все технические детали (например, ошибки, задержки, зависания, требования к действиям и т.п.) и любые вопросы, заданные в сообщении. Не додумывай недостающую информацию.
