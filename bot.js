...
function extractTextContent(message) {
    ...
    return {
        ...
        subject,
        body,
        isReply, // Новое поле, указывающее, что сообщение является ответом
        isError,
...
async function summarizeMessages(messages, lastMsgId) {
    try {
        ...
        3. Тема: если тема явно указана или может быть определена из контекста, укажи её. Если сообщение является ответом (RE), укажи, что тема берётся из контекста предыдущего сообщения.
