...
async function summarizeMessages(messages, token, lastMsgId) {
    try {
        ...
        Привет! Ниже представлено одно сообщение из Teams. Твоя задача — составить точное и понятное резюме этого сообщения, не упуская ни одной важной детали.
        
        Учитывай следующее:
        1. ID сообщения: Обязательно укажи уникальный идентификатор сообщения.
        2. Отправитель: Определи имя и контакт (например, email). Если указана должность или компания, упомяни их, но не включай их в основное содержание.
        3. Тема: Если в сообщении указана тема или если её можно определить из контекста, обязательно укажи её. Если сообщение является ответом (с пометкой "RE"), отметь, что тема взята из контекста предыдущего сообщения.
        4. Содержание: Четко опиши основное содержание сообщения в одном-двух предложениях. Если в сообщении присутствуют технические детали (например, коды ошибок, время, ID, упоминание конкретных элементов, таких как "залагала кнопка входа" или требование очистки кэша), обязательно включи их в описание. Не додумывай недостающие детали.
        5. Игнорируй элементы, не относящиеся к сути (например, подписи, ссылки на Telegram и прочее), если они не влияют на понимание проблемы.
        6. Если присутствуют дополнительные фразы вроде "и еще 1", постарайся определить, относится ли это к сути сообщения или является дополнительной информацией, и отрази это в резюме.
        
        Пример ожидаемого результата:
        ID: 12345  
        Отправитель: Дмитрий Селиванов (d.selivanov@sportxline.com)  
        Тема: Проблемы с админкой  
        Содержание: Айбек сообщает, что админка не работает. По видео видно, что кнопка входа зависла, и, вероятно, требуется очистка кэша браузера. Технические детали: упоминается проблема с кнопкой входа, что может быть вызвано зависанием, а также необходимость очистки кэша.
        
        Составь резюме для следующего сообщения:
