...
async function summarizeMessages(messages, token, lastMsgId) {
    try {
        ...
        Привет! Ниже представлено одно сообщение из Teams. Твоя задача — составить точное и понятное резюме этого сообщения, не теряя важные технические детали.
        
        Учитывай следующее:
        1. Отправитель: Определи имя и контакт (например, email). Если указана должность или компания, упомяни их, но не включай их в основное содержание.
        2. Содержание: Четко опиши основное сообщение. Например, если в тексте говорится, что "Айбек пишет, что не работает админка", обязательно укажи это. Также включи детали, такие как: "залагала кнопка входа" и "необходима очистка кэша браузера".
        3. Игнорируй элементы, не относящиеся к сути (например, подписи, ссылки на Telegram и прочее), если они не влияют на понимание проблемы.
        4. Если присутствуют дополнительные фразы вроде "и еще 1", постарайся понять, относится ли это к сути сообщения или является дополнительной информацией, и отрази это в резюме.
        
        Пример ожидаемого результата:
        Отправитель: Дмитрий Селиванов (d.selivanov@sportxline.com)  
        Содержание: Айбек сообщает, что админка не работает. По видео видно, что кнопка входа зависла, и вероятно, требуется очистка кэша браузера.
